...
#ifdef USE_WEBSERVER_ASSETS
// Forward declaration of the asset table generated into main.cpp by __init__.py.
// Defined in namespace esphome::web_server_idf with extern const for external linkage.
extern const EmbeddedAsset ESPHOME_ASSET_TABLE[];

// Serves every flash-embedded static asset (fonts, icon CSS, images) from the
// ESPHOME_ASSET_TABLE generated by web_server_idf/__init__.py. Registered before
// the WebServer handler so /assets/* requests are handled here, not 404'd.
class EmbeddedAssetsHandler : public AsyncWebHandler {
 public:
  bool canHandle(AsyncWebServerRequest *request) const override {
    if (request->method() != HTTP_GET) return false;
    char url_buf[AsyncWebServerRequest::URL_BUF_SIZE];
    auto url = request->url_to(url_buf);
    for (size_t i = 0; i < USE_WEBSERVER_ASSETS_COUNT; i++) {
      if (url == ESPHOME_ASSET_TABLE[i].url) return true;
    }
    return false;
  }
  ...
  void handleRequest(AsyncWebServerRequest *request) override {
    ...
    auto url = request->url_to(url_buf);
    for (size_t i = 0; i < USE_WEBSERVER_ASSETS_COUNT; i++) {
      const auto &a = ESPHOME_ASSET_TABLE[i];
      if (url != a.url) continue;
      httpd_req_t *req = *request;
      httpd_resp_set_status(req, HTTPD_200);
      httpd_resp_set_type(req, a.content_type);
      httpd_resp_set_hdr(req, "Accept-Ranges", "none");
      httpd_resp_set_hdr(req, "Cache-Control",
                         strncmp(a.url, "/assets/", 8) == 0 ? "public, max-age=31536000, immutable" : "no-cache");
      if (a.gzipped) httpd_resp_set_hdr(req, "Content-Encoding", "gzip");
      for (const auto &header : DefaultHeaders::Instance().headers()) {
        httpd_resp_set_hdr(req, header.name, header.value);
      }
      // Stream in 512-byte chunks to keep stack usage minimal
      size_t remaining = a.size;
      const uint8_t *ptr = a.data;
      while (remaining > 0) {
        size_t chunk = remaining < 512 ? remaining : 512;
        httpd_resp_send_chunk(req, reinterpret_cast<const char *>(ptr), chunk);
        ptr += chunk;
        remaining -= chunk;
      }
...
void AsyncWebServer::begin() {
  ...
  // Register embedded asset handler as the first handler so /assets/* requests
  // are served from flash before any WebServer handler gets a chance to 404 them.
#ifdef USE_WEBSERVER_ASSETS
  static EmbeddedAssetsHandler assets_handler;
  if (this->handlers_.empty() || this->handlers_[0] != &assets_handler) {
    this->handlers_.insert(this->handlers_.begin(), &assets_handler);
  }
#endif
