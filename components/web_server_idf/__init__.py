...
async def to_code(config):
    ...
    # ── www.js ────────────────────────────────────────────────────────────────
    # Embed the per-device www.js as the first asset, served at /0.js.
    # Resolved via _COMP_DIR so this works for both local Docker builds and
    # remote ESPHome package imports (external_components: type: git).
    device_slug = CORE.config.get("substitutions", {}).get("device_slug", "")
    www_js_path = os.path.join(_WEBSERVER_DIR, device_slug, "www.js")
